...
def conv_block(input_tensor, filters, kernel_size=(3, 3, 3), padding='valid'):
    output_tensor = compose(
        keras.layers.Conv3D(filters, kernel_size, padding=padding),  # 卷积层
        keras.layers.BatchNormalization(momentum=0.8),  # 批量归一化层
        keras.layers.ReLU())(input_tensor)  # ReLu层
    return output_tensor

def discriminator_block(input_tensor, filters, kernel_size=(3, 3, 3), stride=1, padding='same'):
    output_tensor = compose(
        keras.layers.Conv3D(filters, kernel_size, strides=stride, padding=padding),  # 卷积层
        keras.layers.BatchNormalization(momentum=0.8),  # 批量归一化层
        keras.layers.LeakyReLU(alpha=0.2))(input_tensor)  # ReLu层
    return output_tensor
